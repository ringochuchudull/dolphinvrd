...
class Tracker:
    ...
    def regress_tracks(self, blob):
        ...
        #print('test regress tracks', boxes.shape, scores.shape)
    ...
    def step(self, blob, idx):
        ...
        self.visualise_save_result(b=blob, i=idx)
        
        
    # Custom Function
    def visualise_save_result(self, b, i):
        
        input_img = b['img'].cpu().numpy()
        input_img = input_img.squeeze()
        input_img = np.transpose(input_img, (1, 2, 0)) # CHW -> HWC
        
        input_img = np.array(input_img[:, :, ::-1]*255)
        input_img = cv2.UMat(input_img)
        
        abd = np.zeros([1000,1000,3])
        abd = cv2.UMat(abd)
        for t in self.tracks:
            tpos = t.pos[0]
            ...
            p1 = (int(np.rint(tpos[0])), int(np.rint(tpos[1])) )
            p2 = (int(np.rint(tpos[2])), int(np.rint(tpos[3])) )
            
            colour = (int(t.colour[0]), int(t.colour[1]), int(t.colour[2]) )
            
            input_img = cv2.rectangle(input_img, p1, p2, colour, 2)
        
            text_x = int((p1[0] + p2[0])/2)
            text_y = int((p1[1] + p2[1])/2)
            input_img = cv2.putText(input_img,
                                    str(tid),
                                    (text_x, text_y),
        ...
        cv2.imwrite(os.path.join('./testimage', str(i).zfill(6)+'.jpeg'), input_img)
